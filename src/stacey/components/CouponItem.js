...
function CouponItem(props) {
  ...
  //設定進度條的外觀
  let getedCountStyle = {
    background: `linear-gradient(to right, #0dd2c5 ${props.data.cp_getedCount}%, #a0a0a0 ${props.data.cp_getedCount}%)`,
  }
  ...
  let getButton = (
    <button
      disabled={couponState === '發放結束'}
      onClick={() => {
        props.getCoupon(props.data)
      }}
    >
      ...
      <div className={couponClassName}>
        <div className="item">
          ...
          <div className="text">
            <ul>
              <h3>
                {props.data.cpr_discount
                  ? props.data.cpr_discountNum + '元'
                  : props.data.cpr_discountNum + '折'}
              </h3>
            ...
            <div className="state">
              <div
                style={
                  couponState === '發放結束' ? endCouponStyle : getedCountStyle
                }
              ></div>
              {couponState === '發放結束' ? (
                <p>全數領取完畢</p>
              ) : (
                <p>{props.data.cp_getedCount}% 已領取</p>
              )}
            </div>
          </div>
